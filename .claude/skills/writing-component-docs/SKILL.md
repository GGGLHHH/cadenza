...
5. **页面记录了 ≥2 个对外导出的部件时,每个部件按名字可查。** 判据是导出了几个
   部件,不是组件有没有一个主 Root —— 读者查的是「我用的这个部件收什么 props」,
   跟结构无关。默认每个部件一个 H3(`### PartName` + 一行定位 + 迷你表 +
   用法片段)。
   只有当一组部件**签名雷同又没有各自的契约**(清一色 `ComponentProps<'div'>`、
   除 `className` 外无话可说)时,才可以合并成一张表,且表里必须逐个点名 ——
   「其余部件」这种不点名的兜底行不算数。
   任何一个部件有自己的契约(特殊 props、非常规宿主元素、标记部件这类反直觉的
   渲染规则),它就得有自己的 H3 和锚点,哪怕同页正文别处已经讲过。
   家族枢纽页(Field / InputGroup)另外免掉顺序规则行与透传行,第 4 条照旧。
