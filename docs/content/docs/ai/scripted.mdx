...
| `argsChunk` | `number` | 整段 | 工具参数的默认切片（每个 `TOOL_CALL_ARGS` 的字符数）；`tool` 步骤自己的 `argsChunk` 优先 |
| `sse` | `boolean` | `false` | 以 `text/event-stream` 的 `Response`（每个 chunk 一行 `data:`）作答，客户端走真实的 SSE 解析器——与 `/api/ai/chat` 同一条路 |
