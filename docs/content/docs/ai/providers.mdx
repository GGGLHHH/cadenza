...
| `deepseek` | DeepSeek | `true` / `node` | 型号、价格与档位抄自 pi-ai 的生成表（TanStack 没有 DeepSeek adapter） |
...
| `deepseek` | `new DeepSeekChatAdapter(client, model)`——compatible adapter 的子类 | `DEEPSEEK_API_KEY` | `{ thinking: { type }, reasoning_effort }`，low / high / max | 由 `openaiCompatiblePreset` 组成，只换 adapter；`discoverModels` = `GET /models`。子类覆盖两个钩子：`extractReasoning` 把 `delta.reasoning_content` 变成 `REASONING_*` 事件（推理进 `thinking` 部件），`convertMessage` 给每条 assistant 历史回填 `reasoning_content`（DeepSeek 在 thinking 开着的工具回合里没有它会 400） |
...
| `deepseek` | `{ thinking: { type: 'enabled' }, reasoning_effort }`，minimal / medium 折到 low、xhigh 折到 high；非推理模型 `{}` | `{ thinking: { type: 'disabled' } }` | DeepSeek 缺省是开着的 high，所以 off 必须显式发 disabled |
...
`name`（adapter 在事件里报的名字，缺省 = `id`）可选。
...
| `openaiThinking` / `anthropicThinking` / `geminiThinking` / `openrouterThinking` / `grokThinking` / `groqThinking` / `vercelGatewayThinking` / `llmgatewayThinking` / `ollamaThinking` / `openaiCompatibleThinking` / `deepseekThinking` / `noThinking` | `(level, model) => fragment`，各 preset 的映射函数（vertex 复用 `geminiThinking`，mistral / bedrock / byteplus 用 `noThinking`） |
...
| `providers/deepseek` | `deepseek` | `@tanstack/ai-openai`（`/compatible` 子路径） |
