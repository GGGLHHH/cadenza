...
| `deepseek` | `openaiCompatibleText(model, { baseURL: 'https://api.deepseek.com', apiKey: key })` | `DEEPSEEK_API_KEY` | `{ thinking: { type }, reasoning_effort }`，low / high / max | 由 `openaiCompatiblePreset` 组成；`discoverModels` = `GET /models`；thinking 开着时带工具的多轮要回传 `reasoning_content`，compatible adapter 不带——带工具的会话把档位设 `off` |
