...
**剩下的缺口**：`output_text.annotations`（url 引用）仍然被上游丢弃。`Sources` 折叠块里能不能有东西，
取决于 DeepSeek 的 `action` 对象实际带什么字段——真 key 实测前不打包票。
...
而**入站那次改写不会漏到线上**——历史里被改写成 `function_call` 的内建搜索，会按
`metadata.providerExecuted` 认出来（不按工具名，免得撞上消费者自己叫 `web_search` 的函数），
