...
四个动作全部只靠 `useChat` 的返回值与门面的纯函数：复制用 `messageText(message)`，
...
11 个类型一并导出：`TranscriptProviderProps` / `TranscriptProps` / `TranscriptMessageProps` /
...
`TranscriptErrorProps` / `TranscriptInterrupt`。
