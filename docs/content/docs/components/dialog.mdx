...
要拦的退路一共三条：右上角的 ✕、点遮罩、`Esc`。好在 `cancel()` **不分 `reason`**，
一次全挡住 —— 所以这里不需要 `disablePointerDismissal` 或
`showCloseButton={false}`：那两个是永久的，而「锁住」只该持续到请求回来。
[AlertDialog](/docs/components/alert-dialog#异步确认) 那边写法完全一样，只是少了
✕ 那一条。

执行按钮上的 [`pending`](/docs/components/button#进行中) 让按钮保持可聚焦但不再
响应，标签被 spinner 蒙住，宽度不变。

失败时**什么都不用做**：`setOpen(false)` 没被调用，框自然还开着，把错误显示出来
让用户重试即可。
