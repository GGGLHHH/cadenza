...
都是一次普通展开就到底下的部件；它只做两件事：把**按下**和**悬停**在视觉上分开
（见[状态与 className](#状态与-classname)），以及在类型上动两处 —— 把 `className`
...
[Field](/docs/components/field#无障碍) 那页的通道表也因此不收 Toggle。
...
| --- | --- | --- |
| `data-pressed` | `pressed` | 按下。同一时刻 `aria-pressed="true"`；cva 的底色挂在 `aria-pressed` 上，封装层那组挂在 `data-pressed` 上，两条选择器同时命中 |
...
| `value` | `Value extends string`（默认 `string`） | — | **只在 [ToggleGroup](/docs/components/toggle-group) 里有意义**：它是这颗 toggle 在组 value 数组里的身份。独立使用时不用给，也不会出现在 DOM 上 |
...
| `className` | `string` | — | 类名。**只收字符串**，见[状态与 className](#状态与-classname) |
| 其余 | Base UI `Toggle` 的 props（`button` 的原生属性 + `ref`，含 `aria-label`） | — | 透传。`type` 和 `form` 除外：Base UI 把它们吞掉，按钮恒为 `type="button"` |
